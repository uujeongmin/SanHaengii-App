...
import { apiService, Mountain, MountainCourse } from "../data/api";
...
export default function RouteDetailScreen() {
  ...
  const [course, setCourse] = useState<MountainCourse | null>(null);
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        setLoading(true);
        // Fetch all mountains to find the course
        const allMountains = await apiService.getMountains();
        let foundCourse: MountainCourse | null = null;
        let foundMountain: Mountain | null = null;

        for (const m of allMountains) {
          const courses = await apiService.getCourses(m.id);
  ...
  const statCards = [
    ...
    },
    {
      ...
      value: `${record.calories.toLocaleString()} kcal`,
      label: "소모 칼로리",
      color: "#f97316",
    },
    {
      icon: "heart-outline" as const,
      value: `${record.heartRateAvg} bpm`,
      label: "평균 심박수",
      color: "#f43f5e",
    },
    {
          ...
          <View style={styles.chartBox}>
            ...
            >
              <Text style={{ fontSize: 10, color: "#9ca3af" }}>
                출발 ({course.elevationProfile[0]}m)
              </Text>
              <Text style={{ fontSize: 10, color: "#9ca3af" }}>
                최고 ({Math.max(...course.elevationProfile)}m)
              </Text>
            </View>
          </View>
        </View>
